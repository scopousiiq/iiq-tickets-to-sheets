...
| TechnicianPerformance | "How is workload distributed among staff?" | Per-technician: open, closed, aged, avg resolution (calendar days), avg SLA response/resolution (hrs), response breach % and resolution breach % (separate); Window Start/End controls (P2/Q2) scope all metrics to any period (Open/Aged reconstructed as-of window end) |
...
- `TICKET_BATCH_SIZE`: Tickets per page for bulk load (default 1250). SLA lookups are chunked separately at `SLA_CHUNK_SIZE` (500) and are not affected by this value.
